...
second_derivative_dict=Dict("PERIODIC"=>d2_periodic_potential,"QUADRATIC"=>d2_quadratic_potential,"DOUBLE_WELL"=>d2_double_well_potential,"TILTED_DOUBLE_WELL"=>d2_tilted_double_well_potential)
...
for potential in potentials
    ...
    second_derivative=second_derivative_dict[potential]
    ...
    second_order_term(q,p)=(p^2*second_derivative(q,L)-F(q,L)^2)*μ(q,p)/8
    ...
    for dt in dts
        ...
        for method in methods
                ...
                savefig(heatmap(prange,qrange,D,c=:hsv,xlabel="p",ylabel="q",aspect_ratio=ar,title="$(method)",size=(w_side,w_side/aspect_ratio),clims=color_lims_dict[potential]),"./plots/joint/joint_$(method)_$(potential)_$(dt).pdf")#
        ...
        savefig(heatmap(ref_prange,ref_qrange,MU+dt*FOT,c=:hsv,xlabel="p",ylabel="q",aspect_ratio=ar,size=(w_side,w_side/aspect_ratio),clims=color_lims_dict[potential],title="BAOA first order expansion"),"./plots/joint/joint_theoretical_$(potential)_$(dt).pdf")
...
"""        push!(joint_plots,plot(joint_subplots...,layout=(1,length(methods)+1),plot_title="Δt=$(dt)"))
    ...
    savefig(heatmap(ref_prange,ref_qrange,MU,c=:hsv,xlabel="p",ylabel="q",aspect_ratio=ar,size=(w_side,w_side/aspect_ratio),clims=color_lims_dict[potential],title="μ"),"./plots/joint/joint_reference_$(potential).pdf")
