function animate_trajectories(coords,filename;ix=rand(1:length(coords[1])),camera=(30,30))
    ...
    anim=@animate for i=1:n_steps
        ...
        plot([x],[y],[z],seriestype=:scatter,color=:red,xlims=(0,l),ylims=(0,l),zlims=(0,l),label="",markersize=3,showaxis=false,ticks=false,camera=camera)
        plot!(X,Y,Z,seriestype=:scatter,color=:gray,xlims=(0,l),ylims=(0,l),zlims=(0,l),label="",markersize=2,showaxis=false,ticks=false,camera=camera)
        
        for j=1:length(traj_bits_x)
            plot!(traj_bits_x[j],traj_bits_y[j],traj_bits_z[j],color=:red,xlims=(0,l),ylims=(0,l),zlims=(0,l),label="",showaxis=false,ticks=false,camera=camera)
        end
        if i>1
            ...
            if norm([x-lx,y-ly,z-lz])>l/2
                if length(current_traj_bit_x)>1
                    plot!(current_traj_bit_x,current_traj_bit_y,current_traj_bit_z,color=:red,xlims=(0,l),ylims=(0,l),zlims=(0,l),label="",showaxis=false,ticks=false,camera=camera)
                end
